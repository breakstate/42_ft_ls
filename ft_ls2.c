...
/*   Updated: 2017/07/07 17:48:23 by bmoodley         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void printdir(char *path)
{
	...
	{
		while((curr_dir = readdir(dir_ptr)))//read gets d_name
		{
			lstat(curr_dir->d_name, &statbuf);//reads info of d_name into statbuf
			list_add_back(&head, curr_dir->d_name);
		}
	}
	else
		ft_putstr("Not a valid directory\n");
	sort_list(head);//sort
	print_list(head);//print
	//read until first dir (return d_name of first dir?)
